  ...
  prototype : {
    ...
    _loadGlobals : function(req, res, next) {
      ...
      if (config.enablePassport) {
        if (req.user) {
          application.renderOptions.user = req.user;
        } else {
          application.renderOptions.user = false;
        }

        application.renderOptions.isAuthenticated = req.isAuthenticated() || false;
      ...
      application.renderOptions.flash = req.flash;
